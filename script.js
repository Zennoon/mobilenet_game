...
const searched_item_para = document.getElementsByClassName("item")[0];
const result_div = document.getElementsByClassName("result")[0];
const success_button = document.getElementsByClassName("success")[0];
const failure_next = document.getElementsByClassName("next")[0];
...
success_button.addEventListener("click", game);
failure_next.addEventListener("click", game);

let current_prediction = null;
...
async function app()
{
    ...
    const capture_button = document.getElementsByClassName("image-capture")[0];
    capture_button.onclick = async () =>
    {
        ...
        result_div.textContent = "Generated predictions are: ";
        let para;
        for (let item in predictions)
        {
            para = document.createElement("p");
            para.style.margin = "20px";77
            para.textContent += "class-name: " + predictions[item].className + "     probability: " + predictions[item].probability;
            if (predictions[item].className == searched_item)
            {
                para.style.color = "#72f57f";
                success_button.style.display = "block";
            }
            else
            {
                para.style.color = "#e56c45"
            }
            result_div.appendChild(para);
        }
        result_div.style.background = "#fff";
        result_div.style.padding = "20px";
    }
}
...
function get_random_item()
{
    ...
    return (IMAGENET_CLASSES[random_num]);
}

function game()
{
    searched_item = get_random_item();
    searched_item_para.textContent = "Find me a  :   " + searched_item;
    result_div.style.display = "none";
    success_button.display = "none";
}

function change_theme()
{
    console.log(`${theme_button.innerHTML}`);
    if (theme_button.innerHTML == '<i class="fa-solid fa-sun" aria-hidden="true"></i>')
    {
        change_to_light();
    }
    else
...
{
    console.log("Here!")
    theme_button.innerHTML = '<i class="fa-solid fa-moon" aria-hidden="true"></i>';
    body.style.background = LIGHT_COLORS.background;
    body.style.color = LIGHT_COLORS.color;
    header.style.borderBottomColor = "#fff";
}

function change_to_dark()
{
    theme_button.innerHTML = '<i class="fa-solid fa-sun" aria-hidden="true"></i>';
    theme_button.style.color = '#fff';
    body.style.background = DARK_COLORS.background;
    body.style.color = DARK_COLORS.color;
